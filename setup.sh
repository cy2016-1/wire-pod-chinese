#!/bin/bash
#国内可以将https://github.com改为https://hub.njuu.cf/加速下载
...
function getSTT() {
	...
	echo "export PICOVOICE_APIKEY=${picoKey}" >> ./chipper/source.sh
        ...
        if [[ ! -f ./vosk/completed ]]; then
            ...
            wget -q --show-progress --no-check-certificate "https://hub.njuu.cf/alphacep/vosk-api/releases/download/v${VOSK_VER}/${VOSK_ARCHIVE}"
            unzip "$VOSK_ARCHIVE"
            mv "$VOSK_DIR" libvosk
            rm -fr "$VOSK_ARCHIVE"
        ...
        if [[ ! -f ./stt/completed ]]; then
            ...
            if [[ ${ARCH} == "x86_64" ]]; then
                if [[ ${AVXSUPPORT} == "noavx" ]]; then
                    ...
                    wget -q --show-progress --no-check-certificate https://hub.njuu.cf/coqui-ai/STT/releases/download/v1.3.0/native_client.tflite.Linux.tar.xz
                fi
                tar -xf native_client.tflite.Linux.tar.xz
                rm -f ./native_client.tflite.Linux.tar.xz
            elif [[ ${ARCH} == "aarch64" ]]; then
                wget -q --show-progress --no-check-certificate https://hub.njuu.cf/coqui-ai/STT/releases/download/v1.3.0/native_client.tflite.linux.aarch64.tar.xz
                tar -xf native_client.tflite.linux.aarch64.tar.xz
                rm -f ./native_client.tflite.linux.aarch64.tar.xz
            elif [[ ${ARCH} == "armv7l" ]]; then
                wget -q --show-progress --no-check-certificate https://hub.njuu.cf/coqui-ai/STT/releases/download/v1.3.0/native_client.tflite.linux.armv7.tar.xz
